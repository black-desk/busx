...
7. 模块化分层（**不发布 lib**）；后续若做 TUI，可在同一 crate 加第二个 `[[bin]]` 共享 `src/` 模块，或届时再抽 lib。
...
| 出参值格式 | **type-tagged JSON（唯一输出格式）**：`{"type":"<sig>","data":<native>}`；monitor 用 NDJSON。美化/变换一律交给 `busx jq` |
...
**分层原则**：D-Bus 交互、值编解码、JSON 输出、jaq 桥接各居其位；`main.rs` 只做参数解析与分发。后续若做 TUI，可在同 crate 加第二个 `[[bin]]` 共享这些模块，或届时再抽 lib——不为推测性的复用提前付出 lib 边界成本。
...
busx 始终输出**紧凑 JSON**；需要缩进美化或字段变换用 `busx jq`（§11）。`monitor` 流式输出为 NDJSON（§10）。
...
**恒为 NDJSON**，每条消息一行 JSON 对象，便于 `busx monitor | busx jq 'select(...)'`。
